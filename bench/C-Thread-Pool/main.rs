...
pub unsafe extern "C" fn task(mut arg: *mut libc::c_void) {
    let mut tmp: pthread_t = 0;
    tmp = pthread_self();
    printf(
        b"Thread #%u working on %d\n\0" as *const u8 as *const libc::c_char,
        tmp as libc::c_int,
        arg as libc::c_int,
    );
}
unsafe fn main_0() -> libc::c_int {
    ...
    let mut tmp: threadpool = 0 as *mut thpool_;
    let mut i: libc::c_int = 0;
    puts(b"Making threadpool with 4 threads\0" as *const u8 as *const libc::c_char);
    tmp = thpool_init(4 as libc::c_int);
    thpool = tmp;
    puts(b"Adding 40 tasks to threadpool\0" as *const u8 as *const libc::c_char);
    i = 0 as libc::c_int;
    while i < 40 as libc::c_int {
        thpool_add_work(
            thpool,
            Some(task as unsafe extern "C" fn(*mut libc::c_void) -> ()),
            i as uintptr_t as *mut libc::c_void,
        );
        i += 1;
    }
    thpool_wait(thpool);

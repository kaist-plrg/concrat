...
extern "C" {
    ...
    fn pthread_kill(__threadid: pthread_t, __signo: libc::c_int)
     -> libc::c_int;
...
pub union __anonunion___sigaction_handler_363639592 {
    pub sa_handler: Option<unsafe extern "C" fn(_: libc::c_int) -> ()>,
...
pub struct job {
    pub prev: *mut job,
    pub function: Option<unsafe extern "C" fn(_: *mut libc::c_void) -> ()>,
...
pub unsafe extern "C" fn thpool_init(mut num_threads: libc::c_int)
 -> *mut thpool_ {
    ...
    if num_threads < 0 as libc::c_int { num_threads = 0 as libc::c_int }
    ...
    tmp___1 =
        malloc((num_threads as
                    libc::c_ulong).wrapping_mul(::std::mem::size_of::<*mut thread>()
                                                    as libc::c_ulong));
    (*thpool_p).threads = tmp___1 as *mut *mut thread;
    if (*thpool_p).threads as libc::c_ulong ==
    ...
    pthread_mutex_init(&mut (*thpool_p).thcount_lock,
                       0 as *mut libc::c_void as *const pthread_mutexattr_t);
    pthread_cond_init(&mut (*thpool_p).threads_all_idle as
                          *mut pthread_cond_t,
...
pub unsafe extern "C" fn thpool_add_work(mut thpool_p: *mut thpool_,
                                         mut function_p:
...
pub unsafe extern "C" fn thpool_wait(mut thpool_p: *mut thpool_) {
    ...
    loop  {
        if (*thpool_p).jobqueue.len == 0 {
            if (*thpool_p).num_threads_working == 0 { break ; }
        }
        pthread_cond_wait(&mut (*thpool_p).threads_all_idle as
                              *mut pthread_cond_t,
                          &mut (*thpool_p).thcount_lock as
                              *mut pthread_mutex_t);
    }
...
pub unsafe extern "C" fn thpool_pause(mut thpool_p: *mut thpool_) {
    ...
    while n < (*thpool_p).num_threads_alive {
        pthread_kill((**(*thpool_p).threads.offset(n as isize)).pthread,
                     10 as libc::c_int);
        n += 1
    };
}
...
pub unsafe extern "C" fn thpool_num_threads_working(mut thpool_p:
                                                        *mut thpool_)
 -> libc::c_int {
    return (*thpool_p).num_threads_working;
}
unsafe extern "C" fn thread_init(mut thpool_p: *mut thpool_,
                                 mut thread_p: *mut *mut thread,
                                 mut id: libc::c_int) -> libc::c_int {
    let mut tmp: *mut libc::c_void = 0 as *mut libc::c_void;
    ...
    *thread_p = tmp as *mut thread;
    ...
    (**thread_p).id = id;
    pthread_create(&mut (**thread_p).pthread as *mut pthread_t,
                   ...
                   ::std::mem::transmute::<Option<unsafe extern "C" fn(_:
                                           ...
                                           Option<unsafe extern "C" fn(_:
                                                                           *mut libc::c_void)
                                                      ->
                                                          *mut libc::c_void>>(Some(thread_do
                                                                                       as
...
unsafe extern "C" fn thread_hold(mut sig_id: libc::c_int) {
    ...
    while threads_on_hold != 0 { sleep(1 as libc::c_uint); };
}
unsafe extern "C" fn thread_do(mut thread_p: *mut thread)
 -> *mut libc::c_void {
    ...
    let mut act: sigaction =
        sigaction{__sigaction_handler:
                      __anonunion___sigaction_handler_363639592{sa_handler:
                                                                    None,},
    ...
    let mut tmp___0: libc::c_int = 0;
    let mut func_buff:
            Option<unsafe extern "C" fn(_: *mut libc::c_void) -> ()> = None;
    ...
    snprintf(thread_name.as_mut_ptr(), 32 as libc::c_int as size_t,
             b"thread-pool-%d\x00" as *const u8 as *const libc::c_char,
             (*thread_p).id);
    ...
    act.__sigaction_handler.sa_handler =
        Some(thread_hold as unsafe extern "C" fn(_: libc::c_int) -> ());
    tmp___0 =
        sigaction(10 as libc::c_int,
                  &mut act as *mut sigaction as *const sigaction,
                  0 as *mut libc::c_void as *mut sigaction);
    if tmp___0 == -(1 as libc::c_int) {
        fprintf(stderr,
                b"thread_do(): cannot handle SIGUSR1\x00" as *const u8 as
                    *const libc::c_char);
    }
    pthread_mutex_lock(&mut (*thpool_p).thcount_lock);
    (*thpool_p).num_threads_alive =
        ::std::ptr::read_volatile::<libc::c_int>(&(*thpool_p).num_threads_alive
                                                     as *const libc::c_int) +
            1 as libc::c_int;
    pthread_mutex_unlock(&mut (*thpool_p).thcount_lock);
    while threads_keepalive != 0 {
        ...
        if threads_keepalive != 0 {
            ...
            (*thpool_p).num_threads_working =
                ::std::ptr::read_volatile::<libc::c_int>(&(*thpool_p).num_threads_working
                                                             as
                                                             *const libc::c_int)
                    + 1 as libc::c_int;
            pthread_mutex_unlock(&mut (*thpool_p).thcount_lock);
            ...
            if !job_p.is_null() {
                ...
                arg_buff = (*job_p).arg;
                Some(func_buff.expect("non-null function pointer")).expect("non-null function pointer")(arg_buff);
...
unsafe extern "C" fn jobqueue_init(mut jobqueue_p: *mut jobqueue)
 -> libc::c_int {
    let mut tmp: *mut libc::c_void = 0 as *mut libc::c_void;
    ...
    if (*jobqueue_p).has_jobs as libc::c_ulong ==
           0 as *mut libc::c_void as libc::c_ulong {
        return -(1 as libc::c_int)
    }
    pthread_mutex_init(&mut (*jobqueue_p).rwmutex,
                       0 as *mut libc::c_void as *const pthread_mutexattr_t);
...
unsafe extern "C" fn jobqueue_push(mut jobqueue_p: *mut jobqueue,
                                   mut newjob: *mut job) {
    pthread_mutex_lock(&mut (*jobqueue_p).rwmutex);
    (*newjob).prev = 0 as *mut libc::c_void as *mut job;
    match (*jobqueue_p).len {
        0 => { (*jobqueue_p).front = newjob; (*jobqueue_p).rear = newjob }
        _ => {
...
unsafe extern "C" fn jobqueue_pull(mut jobqueue_p: *mut jobqueue)
 -> *mut job {
    let mut job_p: *mut job = 0 as *mut job;
    pthread_mutex_lock(&mut (*jobqueue_p).rwmutex);
...
unsafe extern "C" fn bsem_init(mut bsem_p: *mut bsem,
                               mut value: libc::c_int) {
    if value < 0 as libc::c_int {
        fprintf(stderr,
                b"bsem_init(): Binary semaphore can take only values 1 or 0\x00"
                    as *const u8 as *const libc::c_char);
    ...
    pthread_mutex_init(&mut (*bsem_p).mutex,
                       0 as *mut libc::c_void as *const pthread_mutexattr_t);
    pthread_cond_init(&mut (*bsem_p).cond as *mut pthread_cond_t,
                      0 as *mut libc::c_void as *const pthread_condattr_t);
    (*bsem_p).v = value;
}
...
unsafe extern "C" fn bsem_wait(mut bsem_p: *mut bsem) {
    ...
    while (*bsem_p).v != 1 as libc::c_int {
        pthread_cond_wait(&mut (*bsem_p).cond as *mut pthread_cond_t,
...
pub unsafe extern "C" fn task(mut arg: *mut libc::c_void) {
    ...
    tmp = pthread_self();
    printf(b"Thread #%u working on %d\n\x00" as *const u8 as
               *const libc::c_char, tmp as libc::c_int, arg as libc::c_int);
}
unsafe fn main_0() -> libc::c_int {
    let mut thpool: threadpool = 0 as *mut thpool_;
    let mut tmp: threadpool = 0 as *mut thpool_;
    let mut i: libc::c_int = 0;
    puts(b"Making threadpool with 4 threads\x00" as *const u8 as
             *const libc::c_char);
    tmp = thpool_init(4 as libc::c_int);
    thpool = tmp;
    puts(b"Adding 40 tasks to threadpool\x00" as *const u8 as
             *const libc::c_char);
    i = 0 as libc::c_int;
    ...
    thpool_wait(thpool);
    puts(b"Killing threadpool\x00" as *const u8 as *const libc::c_char);

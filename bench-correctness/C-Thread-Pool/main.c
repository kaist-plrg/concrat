...
#pragma merger("0","/tmp/cil-ET3O4tlo.i","")
...
#pragma merger("0","/tmp/cil-cWFe6gsk.i","")
...
{ 
  ...
  tmp = strtol((char const   * __restrict  )*(argv + 1), (char ** __restrict  )(& p),
               10);
  num_jobs = (int )tmp;
  tmp___0 = strtol((char const   * __restrict  )*(argv + 2), (char ** __restrict  )(& p),
                   10);
  num_threads = (int )tmp___0;
  tmp___1 = thpool_init(num_threads);
  thpool = tmp___1;
  n = 0;
  while (n < num_jobs) {
    thpool_add_work(thpool, (void (*)(void * ))((void *)(& increment)), (void *)0);
    n ++;
  }
  thpool_wait(thpool);
